classdef GuiNetwork
    ...
    methods
%%        
        function r = GuiNetwork(network_type, num_of_nodes, network_size, sensor_range) %constructor to initialize properties
...
%% 
        function [x_cord, y_cord] = generate_net(obj)  % method to generate the WSN topology for simulation
...
function SHI = compute_indirect_info(obj, mali_levels, indirect_mode, diff_value)
    ...
    if (strcmp(indirect_mode, 'co100'))
        ...
        for i = 1:size_diff
            ...
            %copemin = mali_levels - (0.02 * mali_levels);
            %copemax = mali_levels + (0.02 * mali_levels);
            %copemin = mali_levels - (5 * mali_levels);
            %copemax = mali_levels + (5 * mali_levels);
            copemin = mali_levels;
            copemax = mali_levels;
